                ...
                try
                {
                    ...
                    else if (rqFromClient == "ListUser")
                    {
                        ...
                        if (response != null && response.StatusCode == System.Net.HttpStatusCode.OK && !string.IsNullOrEmpty(response.Body))
                        {
                            try
                            {
                                ...
                                StreamWriter writer = new StreamWriter(tcpClient.GetStream());
                                writer.AutoFlush = true;
                                writer.WriteLine(ListUserstring);
                            }
                            catch (Exception ex)
                            {
                    ...
                    else if (rqFromClient == "Listfriend")
                    {
                        ...
                        /*if (response != null && response.StatusCode == System.Net.HttpStatusCode.OK && !string.IsNullOrEmpty(response.Body))
                        {
                            StreamWriter writer = new StreamWriter(client.GetStream());
                            writer.AutoFlush = true;
                            YourDataModel userData = response.ResultAs<YourDataModel>();
                            if (userData.friends != null)
                            {
                                listFriend = userData.friends;
                                foreach (var friend in listFriend)
                                {
                                    listFriendString += friend + "|";
                                }
                                writer.WriteLine(listFriendString);
                            }
                            else if (userData.friends == null)
                            {
